...
This strategy builds on the pilot light strategy but in addition to running a secondary Kubernetes cluster, you also install Gitpod onto it. This further reduces your recovery time, as it removes the installation step during recovery. Gitpod needs to be configured to use the same external dependencies (and thus data) as your regular production installation. You would then point your DNS entry to the secondary cluster to fail over to your secondary cluster.
